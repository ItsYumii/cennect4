...
export abstract class Cell {
    ...
    protected static minX = Number.MAX_SAFE_INTEGER
    protected static minY = Number.MAX_SAFE_INTEGER
    protected static maxX = Number.MIN_SAFE_INTEGER
    protected static maxY = Number.MIN_SAFE_INTEGER
    ...
    protected drawOutlines(cellSize: number, ctx: CanvasRenderingContext2D, mapPos: Vec2, type: Function, cornerJoins = true): number[] {
        let corners = []
        for (const [i, [e1, e2, c]] of Cell.groups.entries()) {
            ...
            const edges = [[e1, !edgeA], [e2, !edgeB]] as const;
            ...
            if (!corner && edgeA != edgeB) for (const [e, exists] of edges) {
                if (!exists) continue
                ...
                ctx.beginPath()
                ctx.moveTo(
                    x + Math.cos(i * Math.PI/2 + shiftRot) * radius * radiusMultiplier,
                    y + Math.sin(i * Math.PI/2 + shiftRot) * radius * radiusMultiplier
                )
                ctx.arc(
                    x, y, radius * radiusMultiplier,
                    i * Math.PI/2 + shiftRot, (i + 1) * Math.PI/2 + shiftRot
                )
                ctx.stroke()
            }
        }
    ...
    protected static add(x: number, y: number, cell: Cell) {
    ...
    public static drawAllCells(cellSize: number, ctx: CanvasRenderingContext2D, mapPos: Vec2): void {
        for (const cy of Cell.map)
            for (const c of cy)
                if (c) c.draw(cellSize, ctx, mapPos);
    }
